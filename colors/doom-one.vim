...
function! s:h(name, ...)
  ...
  " Remove 'bold' attribute from the list of attributes
  let attr_without_bold = substitute(attr, 'bold', '', 'g')
  if !empty(attr_without_bold) && attr_without_bold != 'none'
    let cmd .= ' gui=' . attr_without_bold
    let has_props = v:true
  endif
  if !empty(sp) && sp != 'none'
    let cmd .= ' guisp=' . sp
